...
public class NoiseFlowfield : MonoBehaviour {
        ...
        for (int i = 0; i < _amountOfParticles; i++)
        {
            ...
            while (attempt < 100)
            {
                ...
                if (isValid)
                {
                    ...
                    particleInstance.GetComponent<flowfieldParticle>().parent = this.transform;
                    particleInstance.GetComponent<flowfieldParticle>()._gridSize = _gridSize;
                    particleInstance.GetComponent<flowfieldParticle>()._cellSize = _cellSize;
    ...
    {
        _offset = new Vector3(_offset.x + (_offsetSpeed.x * Time.deltaTime), _offset.y + (_offsetSpeed.y * Time.deltaTime), _offset.y + (_offsetSpeed.z * Time.deltaTime));
